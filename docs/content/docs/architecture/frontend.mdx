...
│   │   ├── departments/       # 部门管理
│   │   ├── api-keys/          # API Key 管理
│   │   ├── github-import/     # GitHub App 导入
│   │   ├── chat-providers/    # 聊天渠道 Provider 管理
│   │   ├── mcp-providers/     # MCP Provider 管理
│   │   ├── settings/          # 系统设置
│   │   ├── tools/             # MCP/Skill/模型配置
...
├── i18n/                      # 国际化配置
│   ├── messages/              # 翻译文件（zh、en、ja、ko、de、es、fr、pt-BR）
│   └── request.ts             # 区域请求处理器
...
| `github-import-api.ts` | GitHub App 仓库导入 |
| `organization-api.ts` | 组织操作 |
| `profile-api.ts` | 用户资料与偏好 |
| `recommendation-api.ts` | 推荐仓库 |
...
- **Tailwind CSS 4** — 用于所有样式的实用优先 CSS 框架
...
前端使用 Vitest + Testing Library，测试文件位于 `lib/__tests__/` 和 `hooks/__tests__/`：
