...
| 管理端点 | `src/OpenDeepWiki/Endpoints/Admin/` | 管理员专用端点（仓库、用户、角色、部门、API Key、工具等） |
| 服务 | `src/OpenDeepWiki/Services/` | 业务逻辑层，涵盖仓库、wiki 生成、聊天、Graphify、管理等 |
| 代理 | `src/OpenDeepWiki/Agents/` | 通过 `AgentFactory` 的 AI 代理编排，支持多个 LLM 提供商 |
| Chat 系统 | `src/OpenDeepWiki/Chat/` | 多平台聊天系统（飞书、QQ、微信、Slack 等 Webhook 集成） |
...
| 测试 | `tests/OpenDeepWiki.Tests/` | xUnit + FsCheck 后端测试；前端 Vitest 位于 `web/lib/__tests__/` |
...
- **最小 API** — 后端使用 ASP.NET Core 最小 API（借助 `Making.MiniApis` 基础设施）而不是控制器，保持端点定义简洁并按域组织
- **多提供商 AI** — `AgentFactory` 抽象了 LLM 提供商差异，通过统一接口支持 OpenAI、Azure OpenAI、Anthropic 和 DeepSeek（含 Responses API 与思考模式）
- **分离的生成模型** — Wiki 生成支持为目录结构、内容生成和翻译使用不同的 AI 模型，允许成本/质量优化
- **后台工作者** — 长时间运行的任务（仓库准备、分支生成、翻译、思维导图、Graphify 产物、增量更新）作为 `BackgroundService` 后台工作者运行
- **扫描计划解析** — 大型仓库按规模自动解析目录树深度与文件数预算（`RepositoryScanPlanResolver`），控制 AI 上下文成本
