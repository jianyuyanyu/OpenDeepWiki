...
| `OpenAIResponses` | OpenAI Responses API（禁用存储输出以兼容第三方端点） |
| `Anthropic` | Anthropic Claude API |
| `DeepSeekOpenAI` | DeepSeek OpenAI 兼容端点（含思考模式支持） |
...
environment:
  ...
  - CHAT_REQUEST_TYPE=DeepSeekOpenAI

  # Wiki 内容生成
  - WIKI_CONTENT_MODEL=deepseek-chat
  - WIKI_CONTENT_ENDPOINT=https://api.deepseek.com/v1
  - WIKI_CONTENT_API_KEY=sk-your-deepseek-key
  - WIKI_CONTENT_REQUEST_TYPE=DeepSeekOpenAI
```

<Callout type="info">
  `DeepSeekOpenAI` 类型内置了思考模式（reasoner 模型）的请求适配。
</Callout>
...
| `WIKI_LANGUAGES` | `en,zh,zh-tw,ja,ko,es,fr,de,pt-br,pl,ru,ar` | 目标语言的逗号分隔列表 |
...
```json title="appsettings.json"
{
  "WikiGenerator": {
    ...
    "MaxRetryAttempts": 3,
    "Languages": "en,zh,zh-tw,ja,ko,es,fr,de,pt-br,pl,ru,ar"
  }
}
```
...
| `ENDPOINT` | 是 | `https://api.routin.ai/v1`（内置默认） | 聊天 AI 端点 |
...
<Callout type="info">
  除环境变量外，管理员还可以在后台「模型管理」中配置 Provider/Model 绑定，系统设置中的绑定优先于环境变量默认值。
</Callout>
