...
- [ ] 1. 创建数据库实体和迁移
  ...
  - [ ] 1.5 更新MasterDbContext添加DbSet和索引配置
...
- [ ] 5. 实现前端悬浮球和对话面板
  ...
  - [ ] 5.3 创建chat-api.ts SSE客户端
    - 在 `web/lib/` 目录下创建
    - 实现streamChat异步生成器函数
    - 解析SSE事件流
    - _Requirements: 9.2_
  - [ ] 5.4 创建FloatingBall悬浮球组件
    - 在 `web/components/chat/` 目录下创建
    - 固定在页面右下角
    - 支持enabled、iconUrl、isOpen属性
    - _Requirements: 1.1, 1.2, 1.3, 1.4, 1.5_
  - [ ] 5.5 创建ChatPanel对话面板组件
    - 包含消息列表、输入框、发送按钮
    - 包含ModelSelector模型选择器
    - 支持Markdown渲染
    - 支持显示工具调用信息
    - _Requirements: 2.1, 2.2, 2.3, 2.5, 2.6_
  - [ ] 5.6 创建ImageUpload图片上传组件
...
- [ ] 6. Checkpoint - 内置悬浮球功能验证
  - 确保所有测试通过，如有问题请询问用户

- [ ] 7. 实现用户应用管理后端
  - [ ] 7.1 创建ChatAppService应用服务
    - 在 `src/OpenDeepWiki/Services/Chat/` 目录下创建
  ...
  - [ ] 7.3 创建AppStatisticsService统计服务
...
- [ ] 9. Checkpoint - 用户应用后端验证
  - 确保所有测试通过，如有问题请询问用户

- [ ] 10. 实现用户应用前端
  - [ ] 10.1 创建apps-api.ts应用API客户端
    - 在 `web/lib/` 目录下创建
...
- [ ] 11. 实现嵌入脚本前端
  ...
  - [ ] 11.2 创建EmbedChatWidget嵌入对话组件
    - 独立的悬浮球和对话面板
    - 使用应用配置的模型
    - _Requirements: 14.5, 14.6_

- [ ] 12. 实现管理后台配置
  - [ ] 12.1 创建AdminChatAssistantService管理服务
    - 实现GetConfigAsync获取配置
    - 实现UpdateConfigAsync更新配置
    - _Requirements: 10.1, 10.6_
  - [ ] 12.2 创建AdminChatAssistantEndpoints管理端点
